...
import { ArrowRight, Zap, MapPin, Shield, Leaf, Clock, ChevronRight, Star, Play, Image as ImageIcon } from 'lucide-react'
import { useReveal } from '@/hooks/useReveal'
import { Section, SectionHeader, FeatureCard, TickerStrip, LiveBadge, GradientDivider } from '@/components/ui/UIComponents'
...
function Hero() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      setCounts({ s:Math.round(targets.s*e), c:Math.round(targets.c*e), u:Math.round(targets.u*e) })
  ...
  return (
    <section ref={heroRef} className="relative min-h-screen overflow-hidden hero-bg" style={{ minHeight:'100svh' }}>
      {/* BG image with parallax */}
      <motion.div className="absolute inset-0" style={{ y: imgY }}>
        <img src={IMAGES.station} alt="" className="w-full h-full object-cover object-center"
          style={{ opacity:.35, filter:'saturate(0.7) brightness(0.7)' }}
          onError={(e) => { e.target.style.display='none' }} />
        {/* Color overlay */}
        <div className="absolute inset-0" style={{
          background: 'radial-gradient(ellipse 90% 70% at 60% 40%, rgba(200,255,0,0.07) 0%, transparent 60%), radial-gradient(ellipse 50% 60% at 10% 80%, rgba(168,212,0,0.04) 0%, transparent 50%)'
        }} />
      </motion.div>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[40%] pointer-events-none"
        style={{ background: 'linear-gradient(to bottom, transparent, #0A0B0A)' }} />

      {/* Content */}
      <motion.div style={{ opacity }} className="relative flex flex-col justify-center min-h-screen pt-[80px] pb-16">
        <div className="max-w-[1440px] mx-auto px-5 lg:px-10 w-full">
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            <div>
              <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ duration:.6 }} className="mb-6">
              ...
              <motion.h1 initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} transition={{ duration:.7, delay:.1 }}
                className="text-hero text-ev-white mb-5" style={{ fontWeight:900 }}>
                Charge fast,<br />
                <span className="text-gradient">Go Far.</span>
              </motion.h1>
              <motion.p initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ duration:.6, delay:.25 }}
                className="text-[1.05rem] text-ev-muted leading-relaxed max-w-[440px] mb-8 font-light">
                {COMPANY.slogan} Ultra-fast, solar-powered EV charging across India.
              </motion.p>
              <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} transition={{ duration:.5, delay:.35 }}
                ...
                <Link to="/services" className="btn-outline">View Charging Plans</Link>
              </motion.div>

              {/* Stats */}
              <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ duration:.6, delay:.45 }}
                ...
                {[
                  ...
                  <div key={i} className="flex flex-col">
                    <span className="font-extrabold leading-none text-ev-white" style={{ fontSize:'clamp(2rem,4vw,2.8rem)', letterSpacing:'-.04em' }}>
                      {s.n}<span className="text-ev-lime">{s.suf}</span>
                    </span>
                    <span className="text-[0.66rem] uppercase tracking-[0.2em] text-ev-muted mt-1">{s.l}</span>
                  </div>
                ))}
                ...
                <div className="glass rounded-xl px-4 py-3 flex items-center gap-3 border border-ev-lime/15">
                  ...
                  <div>
                    <div className="text-[0.58rem] uppercase tracking-[0.18em] text-ev-muted">Live Output</div>
                    <div className="font-bold text-ev-lime text-[1.1rem] leading-tight">{STATS.maxPower}</div>
                  </div>
                </div>
              </motion.div>
            </div>
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 opacity-50">
        <span className="text-[0.58rem] uppercase tracking-[0.25em] text-ev-muted">Scroll</span>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4">
          {stats.map((s, i) => (
            <motion.div key={s.l}
              ...
              <div className="font-extrabold leading-none mb-2 text-gradient" style={{ fontSize:'clamp(2.4rem,4vw,3.6rem)', letterSpacing:'-.04em' }}>
                {s.n}
              </div>
              <div className="text-[0.68rem] uppercase tracking-[0.18em] text-ev-muted">{s.l}</div>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
        <div className="reveal order-2 lg:order-1">
          <div className="relative rounded-2xl overflow-hidden border border-ev-lime/[0.1] bg-ev-card2 aspect-[4/3]">
            ...
            <svg viewBox="0 0 400 380" className="w-full h-full p-5">
              ...
              {[[196,295,145,210],[145,210,200,98],[200,98,270,180],[196,295,200,262],[143,212,132,232],[200,98,155,120],[196,295,209,328]].map(([x1,y1,x2,y2],i) => (
              ...
              {[
                ...
                <g key={n.city}>
                  ...
                  {n.major && <>
                    <circle cx={n.x} cy={n.y} r="16" fill="none" stroke="#C8FF00" strokeWidth="1" opacity=".25"
                      style={{ animation:`mapRing 2.5s ease-out ${n.d} infinite` }} />
                    <text x={n.x+9} y={n.y+4} fill="rgba(200,212,192,0.6)" fontSize="8.5" fontFamily="DM Sans" fontWeight="500">{n.city}</text>
                  </>}
                </g>
              ))}
            </svg>
            <div className="absolute bottom-4 left-4 right-4 glass-dark rounded-xl px-4 py-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[0.66rem] uppercase tracking-[0.15em] text-ev-lime font-semibold">Network Live</span>
              </div>
              <span className="text-[0.72rem] text-ev-muted">{STATS.cities} cities · {STATS.stations} stations</span>
            </div>
          </div>
        </div>
        <div className="order-1 lg:order-2">
          ...
          <h2 className="text-h2 text-ev-white mb-5 font-bold reveal reveal-delay-1">
            Everywhere you drive,<br /><span className="text-gradient">we're there.</span>
          </h2>
          <p className="text-ev-muted leading-relaxed mb-8 font-light reveal reveal-delay-2">
            {STATS.stations} charging stations across {STATS.cities} cities and {STATS.highways} national highway corridors. Methodically placed to cover every major route across India.
          </p>
          <div className="grid grid-cols-2 gap-3 mb-8 reveal reveal-delay-3">
            {[['350+','Total Stations'],['150kW','Peak Power'],['12','Highways'],['99.7%','Uptime SLA']].map(([n,l]) => (
              <div key={l} className="bg-ev-card rounded-xl p-4 border border-ev-lime/[0.08] hover:border-ev-lime/20 transition-colors group">
                <div className="font-bold text-[1.5rem] text-ev-lime leading-tight mb-1 group-hover:text-ev-yellow transition-colors" style={{ letterSpacing:'-.02em' }}>{n}</div>
                <div className="text-[0.68rem] uppercase tracking-[0.12em] text-ev-muted">{l}</div>
              </div>
            ))}
          </div>
  ...
  return (
    <Section dark={false}>
      <div className="flex items-end justify-between mb-10">
        ...
        <Link to="/gallery" className="btn-ghost text-ev-lime hidden sm:flex reveal reveal-delay-2">
          View All <ArrowRight size={14} />
        </Link>
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        {[
          { src: IMAGES.station, label: 'EV Halt Station — Night View',   tag: 'Infrastructure', span: 'lg:col-span-2' },
          { src: IMAGES.brand,   label: 'Your Journey Has Just Begun',     tag: 'Branding',       span: '' },
          { src: IMAGES.logo,    label: 'EV Halt Brand Identity',          tag: 'Brand',          span: '' },
          { src: IMAGES.station, label: 'DC Fast Charger — Close Up',      tag: 'Technology',     span: '' },
          { src: IMAGES.brand,   label: 'Charge Fast, Go Far',             tag: 'Vision',         span: '' },
        ].map((item, i) => (
          <motion.div
            key={i}
          ...
          >
            <img src={item.src} alt={item.label} onError={(e) => { e.target.src = IMAGES.station }} />
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <div className="text-ev-white text-sm font-semibold leading-tight">{item.label}</div>
            </div>
          </motion.div>
        ))}
      </div>
      <div className="mt-6 text-center sm:hidden">
        <Link to="/gallery" className="btn-ghost text-ev-lime">View All Images <ArrowRight size={14} /></Link>
      </div>
    </Section>
  )
}
...
function Features() {
  const list = [
    { icon: Zap,    title: 'Ultra-Fast Charging',   body: `Up to ${STATS.maxPower} DC. Add 100 km range in under 15 minutes at our flagship stations.`, tag: STATS.maxPower },
    ...
    { icon: Shield, title: 'Plug & Charge Ready',   body: 'ISO 15118 auto-authentication. Just plug in — billing happens automatically.', tag: 'ISO 15118' },
    { icon: Clock,  title: `${STATS.uptime} Uptime`, body: 'Real-time monitoring, predictive maintenance, 24/7 response teams. SLA guaranteed.', tag: `${STATS.uptime} SLA` },
  ]
  return (
    <Section dark>
      <SectionHeader label="Why EV Halt" title="The standard for" highlight="EV charging in India"
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {list.map((f, i) => (
          <motion.div key={f.title} initial={{ opacity:0, y:24 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:.55, delay:i*.1 }}>
            <FeatureCard {...f} />
          </motion.div>
        ))}
      </div>
...
function SubscriptionCTA() {
  const plans = [
    { name:'Pay Per Use', price:'₹12',  unit:'per kWh',    features:['No subscription','App & RFID access','Real-time billing','All charger types'], cta:'Start Now',      featured:false },
    { name:'Monthly Pass', price:'₹999', unit:'per month', features:['Unlimited AC sessions','10% DC discount','Priority booking','Monthly invoice'], cta:'Subscribe',     featured:true, badge:'Most Popular' },
    { name:'Fleet Enterprise', price:'Custom', unit:'tailored',    features:['Dedicated depots','Volume pricing','Billing API','Account manager'],    cta:'Contact Sales', featured:false },
  ]
  return (
    <Section dark={false} id="subscriptions">
      <SectionHeader center label="Subscriptions" title="Fast charging at" highlight="fair rates."
      ...
      <div className="grid md:grid-cols-3 gap-4 max-w-[900px] mx-auto">
        {plans.map((plan, i) => (
          <motion.div key={plan.name} initial={{ opacity:0, y:24 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:.55, delay:i*.1 }}
            ...
            <div className={`text-[0.62rem] uppercase tracking-[0.2em] mb-4 font-semibold ${plan.featured?'text-ev-black/60':'text-ev-muted'}`}>{plan.name}</div>
            <div className={`font-extrabold leading-none mb-1 ${plan.featured?'text-ev-black':'text-ev-white'}`} style={{ fontSize:'clamp(2.2rem,4vw,2.8rem)',letterSpacing:'-.04em' }}>{plan.price}</div>
            <div className={`text-[0.75rem] mb-5 font-light ${plan.featured?'text-ev-black/60':'text-ev-muted'}`}>{plan.unit}</div>
            <div className={`h-px mb-5 ${plan.featured?'bg-ev-black/15':'bg-ev-lime/[0.08]'}`} />
...
const CheckCircle2Icon = ({ plan }) => (
  <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
    <circle cx="6.5" cy="6.5" r="6" stroke={plan.featured?'#0A0B0A':'#C8FF00'} strokeWidth="1.2" />
...
function Testimonials() {
  const reviews = [
    { name:'Riya Sharma',  city:'Bangalore', stars:5, text:`EV Halt transformed my driving experience. I never worry about range — stations are always where I need them. The ${STATS.maxPower} chargers are phenomenal.` },
    ...
    { name:'Priya Nair',   city:'Hyderabad', stars:5, text:'Used EV Halt from Hyderabad to Bangalore on NH-44 — stations every 100 km, all fast, all working. Impressive.' },
  ]
  return (
    <Section dark>
      <SectionHeader center label="Driver Reviews" title="Trusted by" highlight={`${STATS.drivers} EV drivers`} />
      <div className="grid md:grid-cols-3 gap-4">
        {reviews.map((r, i) => (
          <motion.div key={r.name} initial={{ opacity:0, y:24 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:.55, delay:i*.1 }}
            className="bg-ev-card rounded-2xl p-7 border border-ev-lime/[0.08] hover:border-ev-lime/20 transition-colors group">
            <div className="flex mb-4">
              {Array.from({length:r.stars}).map((_,j) => <Star key={j} size={13} className="text-ev-lime fill-ev-lime mr-0.5" />)}
            </div>
            <p className="text-[0.88rem] text-ev-light leading-relaxed mb-6 font-light italic group-hover:text-ev-white transition-colors">"{r.text}"</p>
            <div>
              <div className="text-[0.88rem] font-semibold text-ev-white">{r.name}</div>
...
function CTABand() {
  return (
    <section className="relative overflow-hidden py-20 lg:py-24"
      style={{ background:'linear-gradient(135deg, #C8FF00 0%, #F0FF00 40%, #A8D400 100%)' }}>
      ...
      <div className="relative max-w-[1440px] mx-auto px-5 lg:px-10 flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div className="flex flex-wrap gap-3 flex-shrink-0">
          <Link to="/network" className="inline-flex items-center gap-2 bg-ev-black text-ev-lime font-bold text-[0.88rem] px-8 py-4 rounded-full hover:bg-ev-card transition-all hover:-translate-y-0.5">
            <MapPin size={15} /> Find Nearest Station
          </Link>
          <Link to="/contact" className="inline-flex items-center gap-2 bg-transparent border-2 border-ev-black/30 text-ev-black font-semibold text-[0.88rem] px-7 py-4 rounded-full hover:border-ev-black transition-all">
            Partner With Us
          </Link>
        </div>
      </div>
